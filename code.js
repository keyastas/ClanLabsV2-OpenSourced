...
bot.on('ready', () => {
  ...
  bot.user.setActivity(`Over the Nation!`, { type: 'WATCHING' });
  bot.channels.get(bot.channels.get(`${config.mainChatChannelID}`).id).send(`***Getting back to work!*** <:TFN:638379410487246908>`)
})

bot.on('message', async message => {

  const args = message.content.split(/[ ]+/)
  const verifiedRole = message.guild.roles.find(role => role.name === "XP");
  const verificationCode = ['apple', 'rain', 'dog', 'cat', 'food','yum','pizza','raindrop','snow','birthday','cake','burger','soda','ice','no','yes','orange','pear','plum','kitty','doggo','meme','pie','rap','eminem','nf','mansion','house'];
  ...
  if (message.content.toLowerCase().startsWith(`${config.prefix}verify`)){
    ...
    if (!verifiedRole){
      return message.channel.send(`Sorry ${message.author}, but this guild is missing the \`XP\` role!\n**Please contact someone to add the role!**`).then(message => message.delete(5000));
    }

    if (message.member.roles.exists("name", "XP")){
  ...
  if (message.content.toLowerCase().startsWith(`${config.prefix}xp`)){
    ...
    if (!args[1]){
      ...
      if (!args[2]){
        ...
        return message.channel.send(`Sorry ${message.author}, but you need to provide mw with a number that's less than the max XP--currently set at ${config.maxXP} XP.\n**Adding XP: \`${config.prefix}xp add 1 username1, username2, username3...\`\nRemoving XP: \`${config.prefix}xp remove 1 username1, username2, username3...\`**`).then(message => message.delete(5000));
      }else if (!args[3]){
